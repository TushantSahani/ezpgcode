	...
	class API extends REST {
		...
		*/
		...
		private function getBedsData(){
			...
			// Input validations
			if(!empty($userId)){
					$sql = mysql_query("SELECT pgBedsTable.ID,occupantTable.ID FROM pgBedsTable LEFT JOIN occupantTable ON pgBedsTable.ID=occupantTable.bedId where pgBedsTable.userId = '$userId' and pgBedsTable.ID IN (SELECT ID FROM pgTable where isDeleted='y')", $this->db);
					$resultList=array();
