	...
	class API extends REST {
		...
		*/
		...
		private function getPg(){
			...
			// Input validations
			if(!empty($userId)){
					$sql = mysql_query("SELECT * FROM pgTable WHERE userId = '$userId' and isDeleted ='n'", $this->db);
					$resultList=array();
			...
			$error = array('status' => "Failed", "msg" => "No PGs found");
			$this->response($this->json($error), 400);
		}
		
		private function getPgBrief(){
			// Cross validation if the request method is POST else it will return "Not Acceptable" status
			$jwtToken=$this->checkSession();
			if($this->get_request_method() != "GET"){
				$this->response('',406);
			...
			// Input validations
			if(!empty($userId)){
					$sql = mysql_query("SELECT ID,pg_name,pg_area FROM pgTable WHERE userId = '$userId'and isDeleted ='n'", $this->db);
					$resultList=array();
					if(mysql_num_rows($sql) > 0){
						while($result = mysql_fetch_array($sql,MYSQL_ASSOC))
						{
