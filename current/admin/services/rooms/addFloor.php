<?php
	...
	class API extends REST {
		...
		private function checkLimit($pgId,$floorType){
			// Cross validation if the request method is POST else it will return "Not Acceptable" status
			if(!empty($pgId) and !empty($floorType)){
						$resultLimit = mysqli_query($this->db,"SELECT floorLimit FROM pgTable WHERE id = '$pgId'");
						$resultRowCount = mysqli_query($this->db,"SELECT floorNo FROM pgFloorsTable WHERE pgId = '$pgId'");
						if(mysqli_num_rows($resultLimit)>0){
							...
							if($rowCount>=$limit){
								$error = array('status' => "limit_exceded", "msg" => "floor limit");
								$this->response($this->json($error), 400);
							}elseif($sameFlag){
								$error = array('status' => "same_floor", "msg" => "same floor");
								$this->response($this->json($error), 400);
							}
							else{
								return true;
        ...
        *******/
        private function addFloor(){
			...
			$floorNo = $this->_request['floorNo'];
			$userId = $jwtToken->uid;
			$pgIds = $jwtToken->pgIds;
			$this->checkPgId($pgIds,$pg_id);
			
			// Input validations
			if(!empty($pg_id) and !empty($floorNo) and !empty($userId)) {
					$this->checkLimit($pg_id,$floorNo);
					// Set autocommit to off
					mysqli_autocommit($this->db,FALSE);
					$addPGStmt = $this->db->prepare("INSERT into pgFloorsTable (pgId,userId,floorNo) VALUES (?,?,?)");
					
					$addPGStmt->bind_param("iis", $pg_id,$userId,$floorNo);
					if($addPGStmt->execute()){
							//$pgId=mysqli_insert_id($this->db);
							$success = array('status' => "Success", "msg" => "Floor Added Successfully");
							mysqli_commit($this->db);
							$this->response($this->json($success), 200);
						
					}else{
					     mysqli_rollback($this->db);
					    $error = array('status' => "Failed", "msg" => "Floor Add Failed");
					    $this->response($this->json($error), 400);
					}
			}
