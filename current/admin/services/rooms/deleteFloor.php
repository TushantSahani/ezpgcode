<?php
	...
	class API extends REST {
		...
		private function checkRoom($pgId,$floorId){
			// Cross validation if the request method is POST else it will return "Not Acceptable" status
			if(!empty($pgId) and !empty($floorId)){
        ...
        *******/
        private function deleteFloor(){
			if($this->get_request_method() != "POST"){
				$this->response('',406);
			}
			$jwtToken=$this->checkSession();
			if(!$jwtToken->admin){
				$failed = array('status' => "fail", "msg" => "Only admin can delete Floor's");
				$this->response($this->json($failed), 401);
			}
			...
			// Input validations
			if(!empty($floorId) and !empty($userId)) {
					$this->checkRoom($pg_id,$floorId);
					$deleteFloorStmt = $this->db->prepare("Delete from pgFloorsTable where userId=? and ID=? and pgId=?");
					$deleteFloorStmt->bind_param("iii", $userId,$floorId,$pg_id);
					if($deleteFloorStmt->execute()){
							//$pgId=mysqli_insert_id($this->db);
							$success = array('status' => "Success", "msg" => "Floor Deleted Successfully");
							//mysqli_commit($this->db);
							$this->response($this->json($success), 200);
						
					}else{
					    $error = array('status' => "Failed", "msg" => "Floor Delete Failed");
					    $this->response($this->json($error), 400);
					}
			}
