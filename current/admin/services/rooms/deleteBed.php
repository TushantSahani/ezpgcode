<?php
	...
	class API extends REST {
		...
		private function checkAssigned($pgId,$bedId){
		...
		*/
        ...
        /****** delete bed 
        *******/
        private function deleteBed(){
			if($this->get_request_method() != "POST"){
				$this->response('',406);
			}
			$jwtToken=$this->checkSession();
			if(!$jwtToken->admin){
				$failed = array('status' => "fail", "msg" => "Only admin can delete Bed's");
				$this->response($this->json($failed), 401);
			}
			...
			if(!empty($roomId) and !empty($userId)) {
					//$this->checkAssigned($pg_id,$bedId);
					$deleteFloorStmt = $this->db->prepare("Delete from pgBedsTable where userId=? and ID=? and pgId=?");
					$deleteFloorStmt->bind_param("iii", $userId,$bedId,$pg_id);
					if($deleteFloorStmt->execute()){
							//$pgId=mysqli_insert_id($this->db);
							$success = array('status' => "Success", "msg" => "Bed Deleted Successfully");
							//mysqli_commit($this->db);
							$this->response($this->json($success), 200);
						
					}else{
					    $error = array('status' => "Failed", "msg" => "Bed Delete Failed");
					    $this->response($this->json($error), 400);
					}
			}
